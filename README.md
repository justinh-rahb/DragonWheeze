...
- **Reading the display (no longer blind!)**: The front panel is driven by a **TM1621C** (HT1621-compatible) LCD controller over a 3-wire serial link (CS / WR / DATA). DragonWheeze **passively sniffs** that link — three GPIOs listen only, never driving anything — decodes the controller's display RAM, and reconstructs exactly what's on screen: the **ambient temperature, humidity, and time-remaining** the dryer already shows. This is how ambient sensing works now, with **zero bus contention** (see the E0 note below). Setpoint changes are still driven open-loop via the reset-then-pulse routine below, but the resulting screen is now fully observable.
- **Reset-before-automation**: To set a target temperature or time deterministically, DragonWheeze runs a reset routine: power OFF, power ON (landing on the known base state), then rapid `M`/`A` pulses to dial in the target profile. (Temps are the fixed set {40, 45, 50}°C; timer is 6–48h in 2h steps.)
- **The E0 Error — AVOIDED by not touching I2C**: Tapping the ESP onto the SH01 mainboard's own AHT20 **I2C** bus makes the board throw `E0` — confirmed electrical (pin capacitance + ESD leakage loading the mainboard's weak pull-ups), not firmware. Rather than splice a dedicated sensor, DragonWheeze sidesteps the bus entirely by reading the dryer's **own** temp/humidity **off the LCD** (above). The legacy AHT20 I2C path remains in the code but defaults to **disabled (`0`)** and is not used.
...
|---|---|---|
| **Optocoupler P** | `GPIO 5` | Power button pulse (active HIGH) |
| **Optocoupler M** | `GPIO 6` | Mode button pulse (active HIGH) |
| **Optocoupler A** | `GPIO 7` | Adjust button pulse (active HIGH) |
| **LCD /CS tap** | `GPIO 3` | TM1621C U1 pin 6, via 3 kΩ series — input, listen only |
| **LCD DATA tap** | `GPIO 4` | TM1621C U1 pin 8, via 3 kΩ series — input, listen only |
| **LCD /WR tap** | `GPIO 10` | TM1621C U1 pin 7, via 3 kΩ series — input, listen only |
