...
esp_err_t dw_board_init(void)
{
    ...
    // Optocouplers are ACTIVE-LOW (LED lit when the GPIO is driven LOW). Idle
    // HIGH = off; a pull-up keeps them off through the pre-init boot window so
    // the pads aren't held down at startup.
    ...
    gpio_set_level(DW_GPIO_OPTO_M, 1);
    gpio_set_level(DW_GPIO_OPTO_A, 1);
    gpio_set_level(DW_GPIO_OPTO_P, 1);
